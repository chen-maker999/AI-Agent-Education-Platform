...
async def chunk_text(request: ChunkRequest):
    ...
    P11 优化：对于教材类内容，优先使用语义分块保持章节完整性
    """
    start_time = datetime.now()

    # P11 优化：对于教材类内容，使用语义分块
    if request.content_type in ["教材", "textbook", "教材/文档"]:
        # 使用语义分块
        semantic_chunks = semantic_chunking_func(
            text=request.content,
            max_chunk_size=request.chunk_size or 1500,
            min_chunk_size=200,
            overlap_ratio=0.15,
        ...
        for i, chunk in enumerate(semantic_chunks):
            chunk_data = {
                ...
                "keywords": [],
                "metadata": {**request.metadata, **chunk.get("metadata", {})},
                "created_at": datetime.utcnow().isoformat()
            }
